...
from objects.laser import collide
...
def redraw_window(window, background, li, le, param_font_color, enemies_list, player, lost, score):
    ...
    score_label = main_font.render(f"Score : {score}", 1, param_font_color)
    ...
    if lost:
        ...
        window.blit(lost_label,
                    (int(window.get_width() / 2 - lost_label.get_width() / 2), int(window.get_height() / 2))
                    )
...
def play_game():
    ...
    while run:
        ...
        redraw_window(window=WIN, background=BG, li=lives, le=level,
                      ...
                      player=player, lost=lost, score=score)
        ...
        if len(enemies) == 0:
            level += 1
            wave_length += 5
            enemies = [Enemy(random.randrange(50, WIDTH - 100),
        ...
        # Udpate enemy postition and enemy lasers
